...
export default {
    ...
    methods: {
        ...
        setBgc(id, color) {
            noteService.setBgc(id, color).then(this.loadNotes)
        },
        setTxt(id, txt) {
            noteService.setTxt(id, txt).then(this.loadNotes)
        },
        setTodos(id, title, todos) {
            noteService.setTodos(id, title, todos).then(this.loadNotes)
        },
        setAnimatedNote(id, title, url) {
            noteService.setAnimatedNote(id, title, url).then(this.loadNotes)
        },
        setPinnedNote(id) {
            noteService.setPinnedNote(id).then(this.loadNotes)
        },
        duplicateNote(id) {
            noteService.duplicateNote(id).then(this.loadNotes)
        },
    },
    watch: {
        '$route.params.mail': {
            handler() {
                ...
                if (!mail) return
                noteService.noteFromMail(JSON.parse(mail)).then(this.loadNotes)
            },
            immediate: true,
        },
    },
    computed: {
        notesToShow() {
            ...
            let notesToShow = this.notes.filter(note => {
                return (
                    !note.isPinned &&
                    (note.type === this.filterBy.type || this.filterBy.type === 'all') &&
                    (note.info.title.toLowerCase().includes(searchStr) || JSON.stringify(note).toLowerCase().includes(searchStr))
                )
            })
